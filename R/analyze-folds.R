...
analyze_folds =
  function(splits, outcome, exposures,
    ...
    for (group_i in seq_along(exposure_groups)) {
      ...
      if (verbose) {
        cat("Analyzing exposure group:", names(exposure_groups)[group_i], "\n")
      }
      ...
      if (class(mixture_train) == "try-error") {
        cat("Error predicting mixture on training data\n")
        print(mixture_train)
        browser()
      }
      
      if (sd(mixture_train) == 0) {
        cat("Error: mixture applied to training data no variation.")
        browser()
      }
      ...
      if (length(unique(quantiles)) < length(quantiles)) {
        ...
        stop(paste0("Error: mixture quantiles are not unique on the training data. ",
                    ...
                    "function, or reduce the number of CV-TMLE folds.")
        )
      }
